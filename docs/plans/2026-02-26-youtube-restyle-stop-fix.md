# YouTube Restyle + Stop Fix — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Fix the Stop button so it kills the full process tree (including ffmpeg children), and restyle the app to match YouTube's dark theme.

**Architecture:** Single-file change — `app.py` only. No new dependencies. Stop fix uses `taskkill /F /T /PID` (Windows built-in). Restyle swaps cyan → YouTube red, Courier New → Arial, and replaces the text header with a canvas-drawn YouTube logo.

**Tech Stack:** Python 3.x, tkinter, subprocess (stdlib only)

---

### Task 1: Fix `_stop()` — kill full process tree

**Files:**
- Modify: `app.py` — `_stop()` method only

**Context:** The current `_stop()` calls `proc.terminate()` which only kills `yt-dlp.exe` directly. yt-dlp spawns `ffmpeg.exe` as a child for audio conversion — that child keeps running after terminate. `taskkill /F /T /PID` kills the parent AND all descendants.

**Step 1: Edit `_stop()` in `app.py`**
...
```bash
git add app.py
git commit -m "fix: kill full process tree on stop (taskkill /F /T /PID)"
```

---

### Task 2: YouTube Dark restyle — full `app.py` rewrite

**Files:**
- Modify: `app.py` — complete replacement

**Context:** All visual changes are in the constants block and `_build_ui` / `_radio` / `_label` methods. Logic methods (`_start`, `_stop`, `_download`, `_browse`, `_log`) are **unchanged** from Task 1. Replace the entire file with the content below.

**Step 1: Replace `app.py` entirely**

Write this exact content to `app.py`:
...
Visually verify:
- Window title is `"YouTube Downloader"`, size ~660×540
- Header shows a red rectangle with a white play triangle, "YouTube Downloader" in white Arial
- Red separator line below header
- All labels/buttons use Arial (not monospace)
- Download button is YouTube red (`#FF0000`) with white text and a `▶` icon
- Stop button is dark gray (`#272727`) with white text and a `■` icon
...
```bash
git add app.py
git commit -m "feat: YouTube dark restyle — red accent, Arial font, logo header"
```
